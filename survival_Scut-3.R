	...
	ODCut = 0.8
	fileBase <- "SURV_var_S_const_OD.0.8_TL.0_TU.5000_+_"
	
	# SURV_var_S_const_OD.0.8_TL.0_TU.5000_+_.nsamples
	#declare functions
	...
	Sdata <- list(); surv <- list(); avg <- list(); mean <- list(); var <- list(); CI <- list()
	
	colours = c("red","blue","green","yellow","black","purple", "cyan","magenta", "orange", "pink", "brown", "violet")
	limitArray = array(); meanOut = array(); nsamples = array();
	nsamples <- read.csv(file=paste(fileWriteLocation, fileBase, ".nsamples", sep=""), head=FALSE, sep=",")
	for (j in c(1:loopLimit)) {
		BEDH <- read.csv(file= paste(fileLocation, fileBase, ".S", names$V1[j], ".csv", sep=""), head=TRUE, sep=" ")
	 	Sdata[[j]] <- makeSurvival(BEDH, ODCut, 0)
		...
		limitArray[j] = names$V1[j]
		meanOut[j] <- paste(names$V1[j], mean[[j]], (mean[[j]] - CI[[j]][1]), (CI[[j]][2] - mean[[j]]), nsamples[j], sep=" ")
	}
	
	legend(x=300,y=0.8,legend=limitArray,col=colours,lty=1)
	
	#write mean and CI to csv file
	titles = paste("S", "mean_r", "L", "U", sep=" ")
	write.table(titles, file = paste(fileLocation, fileBase,".mean.csv", sep=""), sep = " ", col.names = FALSE, append="FALSE", row.names=FALSE, quote=FALSE)
