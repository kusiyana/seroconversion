...
# algorithm variables
cut = 0.8
nSamples = 4 # include at least nSamples
...
ODvalues <- array(c(seq(0.3, 2.9, by=.1)))
...
get_indexTnt <- function(Sdata, TntL, TntU) { # get index of To 
	d<-as.data.frame(Sdata)
	d4 = d
	n <- dim(d)[1] - 1
	nnu = d$ID[n]
...
# write sas start
N <- array(); nrecents = array()
for (i in c(1:loopLimit)) {
	Sdata[[i]] <- make_scenario(BEDH, ODvalues[i], nSamples, timeCutLower, timeCutUpper, ExcludeAlreadySeroconverted, ExcludeNeverSeroconverted, Wmax)
	Strans[[i]] <- Sdata[[i]]
	...
	nrecents[i] <- get_recents(Sdata[[i]], ODvalues[i])
...
write.table(nrecents, file = paste(fileWriteLocation2, ".nrecent", sep=""), sep = ",", col.names = FALSE, append="FALSE", row.names=FALSE, quote=FALSE)
write.table(ODvalues, file = paste(fileWriteLocation2, ".names", sep=""), sep = ",", col.names = FALSE, append="FALSE", row.names=FALSE, quote=FALSE)
