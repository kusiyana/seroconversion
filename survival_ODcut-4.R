	...
	fileBase <- "SURV_04.08_var_OD_const_S.4_TL.0_TU.120_EASF.ENST_OD" # EASF.ENST
		...
		UboundTemp <- NA
		 ...
		 for (j in c(1:nnu)) {
			...
			if ((dim(d[d$ID == j & d$OD > cut,])[1] == 0) & test1 == FALSE) {
				dataOut[j,1] <- j
				dataOut[j,2] <-  integrationWindow - 1 #max(d[d$ID == j & d$OD < cut ,]$t) -  min(d[d$ID == j & d$OD < cut ,]$t)
				dataOut[j,3] <- integrationWindow # 1999
				dataOut[j,4] <- 2
				test2 <- TRUE
			}
			...
			# sanity check
			if (is.numeric(dataOut[j,2]) & is.numeric(dataOut[j,3])) {
				if (dataOut[j,2] > dataOut[j,3]) {
					dataOut[j,2] = dataOut[j,3] - 1
				}
				if (dataOut[j,2] == dataOut[j,3]) {
					dataOut[j,2] = dataOut[j,3] - 1
				}
...
#	 	}
		...
		meanOut[j] <- paste(names$V1[j], mean[[j]], (mean[[j]] - CI[[j]][1]), (CI[[j]][2] - mean[[j]]), SD[[j]], CoeffVar[[j]], nsamples[j], nOverWMax$V1[j], sep=" ")
	}
