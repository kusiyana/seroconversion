	...
	fileBase <- "SURV_var_Tail_const_S.2_OD.0.8_TL.0_TU.5000_EAST.ENSF_Tail"
		ODCut = 0.8
	#declare functions
	
	makeSurvival <- function(dataNNu.Time.OD, cut, ExcludeSetting = 4, tailCut) {
		# settings                               
		# cut = OD cut off
	...
	for (j in c(1:loopLimit)) {
		BEDH <- read.csv(file= paste(fileLocation, fileBase, ".", names$V1[j], ".csv", sep=""), head=TRUE, sep=" ")
	 	Sdata[[j]] <- makeSurvival(BEDH, ODCut, 0, names$V1[j])
		...
		limitArray[j] = paste("Tail.",names$V1[j], sep="")
...
headings = paste("Tail", "Mean_R", "-", "+", "SD", "coV", sep = " ")
