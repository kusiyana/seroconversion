	...
	fileBase <- "SURV_04.08_var_OD_const_S.3_TL.0_TU.140_EASF.ENST_OD"
	integrationWindow = 1999
	...
	for (j in c(1:loopLimit)) {
		BEDH <- read.csv(file= paste(fileLocation, fileBase, ".", names$V1[j], ".csv", sep=""), head=TRUE, sep=" ")
	 	...
	 	for (k in c(1:dim(Sdata[[1]])[1])) {
	 		if(Sdata[[j]][k,2] > integrationWindow) Sdata[[j]][k,2] <- integrationWindow
	 	}
