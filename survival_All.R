...
limitArray = array()
BEDH <- read.csv(file= paste(fileLocation, fileName, sep=""), head=TRUE, sep=" ")
for (j in c(1:1)) {
 	Sdata[[j]] <- makeSurvival(BEDH, limit, 0)
	...
	limitArray[j] = tvalues[j]
	limit <- limit + 0.1
}

legend(x=300,y=0.8,legend=limitArray,col=colours,lty=1)

#fit  <- survfit(Surv(Sdata[[3]][1], Sdata[[3]][2] , type='interval'))
